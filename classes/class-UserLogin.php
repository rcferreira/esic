...
class UserLogin
{
	...
	public function check_userlogin () {
		...
		$query = $this->db->query( 
			'SELECT * FROM ger_usuario WHERE c_usuario = ? LIMIT 1', 
			array( $user ) 
		);
		...
		$user_id = (int) $fetch['n_cod_usuario'];
		...
		if ( $this->phpass->CheckPassword( $user_password, $fetch['c_senha'] ) ) {
			
			// Se for uma sessão, verifica se a sessão bate com a sessão do BD
			if ( session_id() != $fetch['c_session_id'] && ! $post ) { 
				$this->logged_in = false;
			...
			if ( $post ) {
				...
				// Atualiza a senha
				$_SESSION['userdata']['c_senha'] = $user_password;
				
				// Atualiza o ID da sessão
				$_SESSION['userdata']['c_session_id'] = $session_id;
				
				// Atualiza o ID da sessão na base de dados
				$query = $this->db->query(
					'UPDATE ger_usuario SET c_session_id = ? WHERE n_cod_usuario = ?',
					array( $session_id, $user_id )
				);
			}
				
			// Obtém um array com as permissões de usuário
			$_SESSION['userdata']['user_permissions'] = unserialize( $fetch['c_permissoes'] );
